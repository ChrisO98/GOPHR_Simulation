...
class MoveBaseSeq():
    ...
    def __init__(self):
        ...
        rospy.Subscriber("/coordinates", Pose, self.click_callback)
        #self.movebase_client()
        #points_seq = rospy.get_param('move_base_seq/p_seq')
        #msg = Pose()
        #points_seq = [msg.position.x, msg.position.y, msg.position.z]
        #rospy.loginfo(points_seq)
        #print(points_seq)
        #msg = Pose()
        #points_seq = [0,0,0]
        #self.sub = rospy.Subscriber("/coordinates", Pose, self.click_callback)
        #msg = Pose()
        #points_seq = [msg.position.x, msg.position.y, msg.position.z]
        #rospy.Subscriber("/coordinates", Pose, callback=click_callback(msg,points_seq))
        #points_seq = points
        #points_seq = [msg.position.x, msg.position.y, msg.position.z]
        #rospy.loginfo(points_seq)
        #print(points_seq)
        ...
        #self.points_seq = [msg.position.x, msg.position.y, msg.position.z]
        
        
        
    

    def click_callback(self, message: Pose): # message is a pose
        ...
        #callback_pub = rospy.Publisher("callback_y", Pose)
        #callback_pub.publish(self.pose_seq)
